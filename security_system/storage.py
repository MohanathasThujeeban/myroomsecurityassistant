...
def save_owner_profile(profile: OwnerProfile, path: Path = DEFAULT_PROFILE_PATH) -> None:
    ...
    if profile.body_signature is not None:
        payload["body_signature"] = np.asarray(profile.body_signature, dtype=np.float32)
...
def load_owner_profile(path: Path = DEFAULT_PROFILE_PATH) -> OwnerProfile:
    ...
    body_signature = None
    if "body_signature" in data.files:
    ...
    if body_signature is not None and body_signature.ndim != 1:
